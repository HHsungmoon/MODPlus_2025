...
public class TagChain extends TreeSet<SpecInterpretation> implements Comparable<TagChain> {
	...
	private static double maxBYSharedIntensity(ArrayList<Peak> exp, ArrayList<Peak> theo) {
		...
		while (i < exp.size() && j < theo.size()) {
			final Peak a = exp.get(i);   // 실측
			final Peak b = theo.get(j);  // 이론
			final double m1 = a.getMass();
			final double m2 = b.getMass();

			if (Constants.fEqual(m1, m2)) {
				final PeakProperty prop = b.getPeakProperty();
				if (prop == PeakProperty.B_ION || prop == PeakProperty.Y_ION) {
					final double inten = a.getIntensity();
