...
public class DBSearch {
	...
	public static CandidateContainer construct_onemod_cpool(TagPool primitiveTags, double motherMass, TagTrie ixPDB) {
		...
		double minDelta = (Constants.minModifiedMass < 0) ? Constants.minModifiedMass - Constants.gapTolerance[slotIdx]
			: -Constants.gapTolerance[slotIdx];
		double maxDelta = (Constants.maxModifiedMass > 0) ? Constants.maxModifiedMass + Constants.gapTolerance[slotIdx]
			: +Constants.gapTolerance[slotIdx];
		...
		for (Tag tag : longTags) {
			if (tag.get(0).getPeakProperty() != PeakProperty.C_TERM_Y_ION_ONLY
				...
				LinkedList<MODPeptide> bRes = ixPDB.getOneModPeptides(
					tag.getBIonNtermOffset() - Constants.NTERM_FIX_MOD, tag.sequence().toString(),
					tag.getBIonCtermOffset() - Constants.CTERM_FIX_MOD, 0, minDelta, maxDelta,
					Constants.gapTolerance[slotIdx]);
				cpool.addAll(bRes);
			}
			if (tag.get(0).getPeakProperty() != PeakProperty.N_TERM_B_ION_ONLY
				...
				LinkedList<MODPeptide> yRes = ixPDB.getOneModPeptides(
					reverseTag.getYIonNtermOffset() - Constants.NTERM_FIX_MOD, reverseTag.sequence().toString(),
					reverseTag.getYIonCtermOffset() - Constants.CTERM_FIX_MOD, 1, minDelta, maxDelta,
					Constants.gapTolerance[slotIdx]);
				cpool.addAll(yRes);
			}
			realTag++;
	...
	public static CandidateContainer construct_multimod_cpool(TagPool primitiveTags, double motherMass, TagTrie ixPDB) {
		...
		for (Tag tag : longTags) {
			if (tag.get(0).getPeakProperty() != PeakProperty.C_TERM_Y_ION_ONLY
				&& tag.get(3).getPeakProperty() != PeakProperty.N_TERM_Y_ION_ONLY) {
				LinkedList<TagPeptide> bRes = ixPDB.getMultiModPeptides(
					tag.getBIonNtermOffset() - Constants.NTERM_FIX_MOD, tag.sequence().toString(),
					tag.getBIonCtermOffset() - Constants.CTERM_FIX_MOD, 0, minDelta, maxDelta,
					Constants.gapTolerance[slotIdx]);
				cpool.addAll(bRes);
			...
			if (tag.get(0).getPeakProperty() != PeakProperty.N_TERM_B_ION_ONLY
				&& tag.get(3).getPeakProperty() != PeakProperty.C_TERM_B_ION_ONLY) {
				Tag reverseTag = tag.reverseTag();
				LinkedList<TagPeptide> yRes = ixPDB.getMultiModPeptides(
					reverseTag.getYIonNtermOffset() - Constants.NTERM_FIX_MOD, reverseTag.sequence().toString(),
					reverseTag.getYIonCtermOffset() - Constants.CTERM_FIX_MOD, 1, minDelta, maxDelta,
					Constants.gapTolerance[slotIdx]);
				cpool.addAll(yRes);
